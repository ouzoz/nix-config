...
function M:open_clicked ()
  ...
  elseif line > #self.bufs + 1 then
    ...
    if file.is_dir then
      ...
      if row < #self.lines then api.nvim_win_set_cursor(self.state.win, { row + 1, col }) end
    end
  end
end
...
function M:file_tree (dir, old, indent)
  ...
  for _, v in ipairs(dir.childs) do
    ...
    if v.is_dir and v.is_opened then self:file_tree(v, old and old.dirs and old.dirs[v.name], indent + 1) end
  end
end
