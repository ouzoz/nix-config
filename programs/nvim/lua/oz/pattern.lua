...
function Buf:map_exec(key)
    ...
    if self:is_map_ignore(meta) then return end
...
function Buf:is_map_ignore(meta)
    return fn.reg_recording() ~= ''
        or fn.reg_executing() ~= ''
        or fn.visualmode() == '^V'
        or vim.v.count > 0
        -- or (meta.key == keys.cr and fn.pumvisible() ~= 0) -- and fn.complete_info().selected ~= -1)
end
...
local rules = {
    ...
    Bracket('[', ']'),
    Bracket('{', '}'),
    Bracket('<', '>', {'html', 'xml', 'javascriptreact'}),
